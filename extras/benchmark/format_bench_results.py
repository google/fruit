...
parser.add_argument('input_file', metavar='file', type=str, help='The input file where benchmark results will be read from (1 per line, with each line in JSON format). You can use the run_benchmarks.py to run a benchmark and generate results in this format.')
args = parser.parse_args()

with open(args.input_file, 'r') as f:
  bench_results = [json.loads(line) for line in f.readlines()]

def extract_results(fixed_benchmark_params, column_dimension, row_dimension, result_dimension):
  table_data = dict()
      ...
      for param_name, param_value in fixed_benchmark_params.items():
        ...
        if column_value in table_data[row_value]:
          ...
          raise Exception(
            ...
            + 'Consider adding additional values in fixed_benchmark_params. Remaining dimensions: %s vs %s' % (remaining_dimensions, previous_remaining_dimensions))
...
def print_confidence_intervals_table(table_name,
  ...
  for row_header in row_headers:
    table_content += [[row_header_pretty_printer(row_header)]
                      + [value_pretty_printer(table_data[row_header][column_header], min_in_table, max_in_table) if column_header in table_data[row_header]
                         else 'N/A'
                         for column_header in column_headers]]
...
def find_best_unit(units, min_value, max_value):
    ...
    for i in range(len(units)):
        if min_value > units[i]:
            return units[i]
    assert min_value > max(units)
    # Pick the biggest unit
    return units[-1]
...
# The following tables are the ones used in the Fruit wiki, edit this section of the script as needed to generate other tables.

table_data = extract_results({'name': 'fruit_single_file_compile_time'}, column_dimension='num_bindings', row_dimension='compiler_name', result_dimension='compile_time')
print_confidence_intervals_table("Fruit compile time (single file)",
                                 table_data,
                                 column_header_pretty_printer=format_string_pretty_printer('%s bindings'),
                                 value_pretty_printer=time_interval_pretty_printer)

print()
print()

table_data = extract_results({'name': 'fruit_compile_time'}, column_dimension='num_classes', row_dimension='compiler_name', result_dimension='compile_time')
print_confidence_intervals_table("Fruit setup time",
                                 table_data,
                                 column_header_pretty_printer=format_string_pretty_printer('%s classes'),
                                 value_pretty_printer=time_interval_pretty_printer)
print()
print()

table_data = extract_results({'name': 'fruit_run_time'}, column_dimension='num_classes', row_dimension='compiler_name', result_dimension='Total for setup')
print_confidence_intervals_table("Fruit setup time",
                                 table_data,
                                 column_header_pretty_printer=format_string_pretty_printer('%s classes'),
                                 value_pretty_printer=time_interval_pretty_printer)
...
table_data = extract_results({'name': 'fruit_run_time'}, column_dimension='num_classes', row_dimension='compiler_name', result_dimension='Total per request')
print_confidence_intervals_table("Fruit per-request time",
                                 table_data,
                                 column_header_pretty_printer=format_string_pretty_printer('%s classes'),
                                 value_pretty_printer=time_interval_pretty_printer)
...
table_data = extract_results({'name': 'new_delete_run_time'}, column_dimension='num_classes', row_dimension='compiler_name', result_dimension='Total')
print_confidence_intervals_table("New delete time",
                                 table_data,
                                 column_header_pretty_printer=format_string_pretty_printer('%s classes'),
                                 value_pretty_printer=time_interval_pretty_printer)
...
table_data = extract_results({'num_classes': 100}, column_dimension='name', row_dimension='compiler_name', result_dimension='compile_time')
print_confidence_intervals_table("Compile time (100 classes)",
                                 table_data,
                                 column_header_pretty_printer=dict_pretty_printer({'fruit_compile_time': 'Fruit', 'boost_di_compile_time': "Boost.DI"}),
                                 value_pretty_printer=time_interval_pretty_printer)
print()
print()

table_data = extract_results({'num_classes': 100}, column_dimension='name', row_dimension='compiler_name', result_dimension='Total for setup')
print_confidence_intervals_table("Setup time (100 classes)",
                                 table_data,
                                 column_header_pretty_printer=dict_pretty_printer({'fruit_run_time': 'Fruit', 'boost_di_run_time': "Boost.DI"}),
                                 value_pretty_printer=time_interval_pretty_printer)

print()
print()

table_data = extract_results({'num_classes': 100}, column_dimension='name', row_dimension='compiler_name', result_dimension='Total per request')
print_confidence_intervals_table("Per-request time (100 classes)",
                                 table_data,
                                 column_header_pretty_printer=dict_pretty_printer({'fruit_run_time': 'Fruit', 'boost_di_run_time': "Boost.DI"}),
                                 value_pretty_printer=time_interval_pretty_printer)
...
table_data = extract_results({'num_classes': 100}, column_dimension='name', row_dimension='compiler_name', result_dimension='num_bytes')
print_confidence_intervals_table("Executable size (stripped, 100 classes)",
                                 table_data,
                                 column_header_pretty_printer=dict_pretty_printer({'fruit_executable_size': 'Fruit', 'boost_di_executable_size': "Boost.DI"}),
                                 value_pretty_printer=file_size_interval_pretty_printer)
